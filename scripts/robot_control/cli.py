...
def parse_args() -> argparse.Namespace:
    ...
    run.add_argument("--baud", type=int, choices=(9600, 38400), default=9600)
    run.add_argument("--web-port", type=int, default=8765)
    run.add_argument(
        "--no-gamepad",
        action="store_true",
        help="Read-only safety monitor; ARM remains disabled",
    )
...
def run(args: argparse.Namespace) -> int:
    ...
    # dashboard port must have no effect on the HC-06 or RobotRuntime.
